$(function() {
  ...
  const inputHandler = function(e) {
      ...
      $('.movie-button').attr('disabled', inputValue === "");
  }
  source.addEventListener('input', inputHandler);

  // Function to handle click event on enter button
  function handleEnterButtonClick() {
      console.log('Button clicked');
      var my_api_key = '723904577a8194bad54f050e10266af3';
      var title = $('.movie').val();
      if (title === '') {
  ...
  $('.movie-button').on('click', handleEnterButtonClick);
  ...
  $('.click-me-btn').on('click', function() {
    var title = $(this).closest('.movie-card').data('title');
    var my_api_key = '723904577a8194bad54f050e10266af3';
    load_details(my_api_key, title);
  });


  // Other functions (load_details, movie_recs, etc.) remain unchanged
});
